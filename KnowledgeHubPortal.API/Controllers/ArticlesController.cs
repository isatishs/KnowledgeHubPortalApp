...
namespace KnowledgeHubPortal.API.Controllers
{
    ...
    public class ArticlesController : ControllerBase
    {
        ...
        public IActionResult GetAllArticlesForReview(int cid)
        {
            ...
            var allArticlesDto = from article in allArticles
                                 select new ArticleBrowseDTO { ArticleID=article.ArticleID, CategoryID = article.CategoryId, CategoryName = article.Category.Name, Description = article.Description, Title = article.Title, URL = article.URL, PostedBy = article.SubmitedBy };
            return Ok(allArticlesDto);
        }
        ...
        {
            iRepo.RejectArticle(articleIds);
            return Ok();
        }
    }
}
